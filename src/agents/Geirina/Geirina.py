    ...
    **NB** the "real" instance of environment on which the baseline will be evaluated will be built AFTER the creation
    ...
    def action_transformation(self, action_array, name):
        ...
        if len(action_dict) != 0:
            try:
                ...
                print(
                    "This Is AmbiguousAction: You can only change line status with int or boolean numpy array vector.")
        return topo_action
    ...
    def normalize(self, state):
        ...
        for i, x in enumerate(state[0]):
            if x >= 1e5:
                state[0][i] /= 1e4
            elif x >= 1e4 and x < 1e5:
                state[0][i] /= 1e3
            elif x >= 1e3 and x < 1e4:
                state[0][i] /= 1e2
    ...
    def train(self, env,
        ...
        epsilons = np.append(np.linspace(self.epsilon_start, 0.2, int(self.epsilon_decay_steps / 5)),
                             np.linspace(0.19, self.epsilon_end, int(self.epsilon_decay_steps * 4 / 5)))
        print(len(epsilons))
        count_action = {}
        ...
        with tf.Session(config=config, graph=self.graph) as sess:
            ...
            while time_step < iterations:
                ...
                for step in itertools.count():
                    reward_tot = 0
                    print("Epsilons is ", epsilons[min(time_step, self.epsilon_decay_steps - 1)],
                          epsilons[self.epsilon_decay_steps - 1])
                            ...
                            for i, x in enumerate(line_count):
                                if x == 10 and (False in lines_status_list_bool):
                                    ...
                                    for o, e in [(1, 1), (1, 2), (2, 1), (2, 2)]:
                                        action_transf_dict = self.agent_action_space.reconnect_powerline(line_id=i,
                                                                                                         bus_or=o,
                                                                                                         bus_ex=e)
                                        obs_simulate, reward_simulate, done_simulate, _ = self.observation.simulate(
                                            action_transf_dict)
                    ...
                    if 1.0 not in self.action_next or done_simulate:
                        ...
                        obs_simulate, reward_simulate, done_simulate, infor = self.observation.simulate(
                            action_transf_dict)
                            ...
                            for action in tuple(top_actions):
                                ...
                                if obs_simulate is None:
                                    ...
                                    if not done_simulate and reward_simulate > max_score and not has_danger:
                                        ...
                                        print('current best action: {}, score: {:.4f}'.format(chosen_action,
                                                                                              reward_simulate))
                    ...
                    if reward >= 100:
                        reward_tot = reward / 750
                    else:
                        reward_tot = reward / 15
                    ...
                    if episode % self.verbose_per_episode == 0 and (time_step + 1) % 1 == 0:
                        print(
                            "episode: {}, step: {},  action: {}, loss: {:4f},  reward: {:4f}, score: {:.4f}, tot: {:.4f}\n".
                                format(episode + 1, step + 1, action, loss, reward, self.episode_score_history[episode],
                                       reward_tot))
                        sys.stdout.flush()
