...
from beu_l2rpn.utilities.submission import dict_installed_packages
    ...
    for imported_module_name in all_modules:
            ...
            for (wrong_imports, file) in all_modules[imported_module_name]:
                for wrong_import in wrong_imports:
                    ...
                    print("\t * Possible ERROR detected in the imports. Try replacing:\n\t   \t\t{}\n\t   by\n\t   \t\t{}\n\t   in the file \"{}\"\n\t"
                          "might help you solve this error, but it might be a terrible advice. Have a look there anyway :-)"
        ...
        if imported_module_name not in all_files and imported_module_name not in all_dirs:
            ...
            if not print_head:
                print_head = True
                print(HEARDER)
            print("\t * Possible ERROR could not locate the file \"{0}.py \" (this could also be a folder \"{0}/\")".format(module_name))
            ...
            if not print_head:
                print_head = True
                print(HEARDER)
            print("\t * WARNING could not locate the file \"{0}.py \" (this could also be a folder \"{0}/\")".format(module_name))
