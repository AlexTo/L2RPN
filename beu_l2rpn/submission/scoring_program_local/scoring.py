...
def draw_steps_fig(ax, env_name, step_data, ep_score, ep_share):
    ...
    scenario_name = title_fmt.format(env_name, scenario_dir, ep_score, ep_share)
...
def draw_rewards_fig(ax, env_name, reward_data, step_data):
    nb_timestep_played = int(step_data["nb_timestep_played"])
    chronics_max_timestep = int(step_data["chronics_max_timestep"])
    n_blackout_steps = chronics_max_timestep - nb_timestep_played
    scenario_name = env_name + " " + os.path.basename(step_data["chronics_path"])
...
def cli():
    ...
    parser.add_argument("--logs_in", required=True,
                        help="Path to the runners outputs")
...
def compute_episode_score(key_score,
    ...
    ep_reco_reward = ep_info["reco_reward"]
    ...
    reco_reward = ep_reco_reward

    # Linear interp episode reward to codalab score
    if zero_reward != reco_reward:
        # DoNothing agent doesnt complete the scenario
        reward_range = [best_reward, reco_reward, zero_reward, worst_reward]
...
def main():
    ...
    for env_dir in os.listdir(input_dir):
        ...
        if not os.path.isdir(env_path):
            continue
        episode_names = config[env_dir]["episodes_info"].keys()
        for episode_id in sorted(episode_names):
            ...
            with open(time_json, "r") as f:
                ...
                draw_steps_fig(step_axs[step_ax_y, step_ax_x], env_dir,
    ...
    try:
        ...
        print("Warning: GIF copy failed, no gif will be available")
    # try:
    #     # Copy gifs if any
    #     gif_input = os.path.abspath(input_dir)
    #     gif_output = os.path.abspath(output_dir)
