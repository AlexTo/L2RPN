...
import numpy as np
from beu_l2rpn.nn_builder.pytorch.base_network import BaseNetwork


class CNN(nn.Module, BaseNetwork):
    ...
    def __init__(self, input_dim, layers_info, output_activation=None,
        ...
        self.valid_cnn_hidden_layer_types = {'conv', 'maxpool', 'avgpool', 'adaptivemaxpool', 'adaptiveavgpool',
                                             'linear'}
        self.valid_layer_types_with_no_parameters = [nn.MaxPool2d, nn.AvgPool2d, nn.AdaptiveAvgPool2d,
                                                     nn.AdaptiveMaxPool2d]
        BaseNetwork.__init__(self, input_dim, layers_info, output_activation, hidden_activations, dropout, initialiser,
    ...
    def check_CNN_layers_valid(self):
        ...
        - ["linear", out]
        """
        error_msg_layer_type = "First element in a layer specification must be one of {}".format(
        ...
        for layer in all_layers:
            ...
            assert layer_type_name in self.valid_cnn_hidden_layer_types, "Layer name {} not valid, use one of {}".format(
                layer_type_name, self.valid_cnn_hidden_layer_types)
            if layer_type_name == "conv":
                ...
                if layer[1] != layer[2]: print(
                    "NOTE that your maxpool kernel size {} isn't the same as your stride {}".format(layer[1], layer[2]))
                ...
                assert isinstance(layer[3], int) and layer[3] >= 0, error_msg_conv_layer
                if layer[1] != layer[2]: print(
                    "NOTE that your avgpool kernel size {} isn't the same as your stride {}".format(layer[1], layer[2]))
                ...
                for ix in range(2): assert isinstance(layer[ix + 1], int) and layer[
                    ix + 1] > 0, error_msg_adaptivemaxpool_layer
        ...
        for ix, layer in enumerate(all_layers):
            if rest_must_be_linear: assert layer[
                                               0].lower() == "linear", "If have linear layers then they must come at end"
    ...
    def create_and_append_layer(self, input_dim, layer, list_to_append_layer_to):
        ...
        if layer_name == "conv":
            list_to_append_layer_to.extend(
                [nn.Conv2d(in_channels=input_dim[0], out_channels=layer[1], kernel_size=layer[2],
    ...
    def initialise_all_parameters(self):
        ...
        initialisable_layers = [layer for layer in self.hidden_layers if
    ...
    def check_input_data_into_forward_once(self, x):
        ...
        assert x.shape[
               1:] == self.input_dim, "Input data must be of shape (channels, height, width) that you provided, not of shape {}".format(
            x.shape[1:])
