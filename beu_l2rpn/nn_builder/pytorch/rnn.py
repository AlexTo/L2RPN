...
class RNN(nn.Module, BaseNetwork):
    ...
    def __init__(self, input_dim, layers_info, output_activation=None,
        ...
        BaseNetwork.__init__(self, input_dim, layers_info, output_activation,
    ...
    def check_RNN_layers_valid(self):
        ...
        error_msg_layer_type = "First element in a layer specification must be one of {}".format(
            self.valid_RNN_hidden_layer_types)
    ...
    def incorporate_embeddings(self, x, batch_size, seq_length):
        ...
        non_embedded_data = x[:, :, [col for col in range(x.shape[2]) if
                                     col not in self.columns_of_data_to_be_embedded]].float()
    ...
    def process_output_layers(self, x, batch_size, seq_length):
        ...
        for output_layer_ix, output_layer in enumerate(self.output_layers):
            ...
            if out is None:
                ...
                out = torch.cat((out, temp_output), dim=2)
        return out
