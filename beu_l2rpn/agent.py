...
from beu_l2rpn.utils.data_structures.replay_buffer import ReplayBuffer
from beu_l2rpn.utils.util_functions import get_scalers, create_actor_distribution
...
class BeUAgent(AgentWithConverter):
    ...
    def create_networks(self):
        self.critic_1 = Critic(input_dim=self.state_size * self.num_stack_frames, output_dim=self.action_size).to(
            self.device)
        self.critic_2 = Critic(input_dim=self.state_size * self.num_stack_frames, output_dim=self.action_size).to(
            self.device)
        ...
        self.critic_target_2 = Critic(input_dim=self.state_size * self.num_stack_frames,
                                      output_dim=self.action_size).to(self.device)
        self.actor = Actor(input_dim=self.state_size * self.num_stack_frames, output_dim=self.action_size).to(
            self.device)
    ...
    def pick_action(self, eval_ep, frames=None):
        if frames is None:
            frames = self.frames
        ...
        if eval_ep:
            encoded_act = self.actor_pick_action(frames=frames, eval=True)
            ...
            encoded_act = self.actor_pick_action(frames=frames)
            # added by Sonvx
    ...
    def actor_pick_action(self, frames=None, eval=False):
    ...
    def act(self, observation, reward, done=False):
        ...
        heuristic_act = self.try_heuristic_action()
